...
async function generateMRN() {
  const row = await queryOne('SELECT mrn FROM patients ORDER BY id DESC LIMIT 1');
  if (!row) return 'P00001';
  const num = parseInt(row.mrn.replace('P', '')) + 1;
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({ data: patients, total: countRow.total, page: parseInt(page), limit: parseInt(limit) });
  } catch (err) {
...
router.get('/:id', async (req, res) => {
  try {
    const patient = await queryOne(`SELECT * FROM patients WHERE id=? AND active=1`, [req.params.id]);
    if (!patient) return res.status(404).json({ error: 'Patient not found' });

    const [appointments, records, labs, bills, documents] = await Promise.all([
      query(`SELECT a.*, d.name as doctor_name, d.specialization FROM appointments a
             JOIN doctors d ON a.doctor_id = d.id
             WHERE a.patient_id = ? ORDER BY a.appointment_date DESC, a.appointment_time DESC LIMIT 10`, [patient.id]),
      query(`SELECT mr.*, d.name as doctor_name FROM medical_records mr
             JOIN doctors d ON mr.doctor_id = d.id
             WHERE mr.patient_id = ? ORDER BY mr.visit_date DESC LIMIT 10`, [patient.id]),
      query(`SELECT lt.*, d.name as doctor_name FROM lab_tests lt
             JOIN doctors d ON lt.doctor_id = d.id
             WHERE lt.patient_id = ? ORDER BY lt.requested_at DESC LIMIT 10`, [patient.id]),
      query(`SELECT b.*, (SELECT SUM(amount) FROM payments WHERE bill_id=b.id) as paid_amount
             FROM bills b WHERE b.patient_id = ? ORDER BY b.created_at DESC LIMIT 5`, [patient.id]),
  ...
  try {
    ...
    const r = await run(
      `INSERT INTO patients (mrn,first_name,last_name,dob,gender,blood_type,phone,email,address,
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [mrn, first_name, last_name, dob||null, gender, blood_type, phone, email, address,
       emergency_contact_name, emergency_contact_phone, insurance_provider, insurance_number, allergies, notes]
    );
    await run(
    ...
    const patient = await queryOne('SELECT * FROM patients WHERE id=?', [r.lastId]);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await run(
      `UPDATE patients SET first_name=?,last_name=?,dob=?,gender=?,blood_type=?,phone=?,email=?,
       address=?,emergency_contact_name=?,emergency_contact_phone=?,insurance_provider=?,
       insurance_number=?,allergies=?,notes=? WHERE id=?`,
      [first_name, last_name, dob||null, gender, blood_type, phone, email, address,
       emergency_contact_name, emergency_contact_phone, insurance_provider, insurance_number,
    ...
    res.json(await queryOne('SELECT * FROM patients WHERE id=?', [req.params.id]));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    const patient = await queryOne('SELECT * FROM patients WHERE id=? AND active=1', [req.params.id]);
    if (!patient) return res.status(404).json({ error: 'Patient not found' });
    await run('UPDATE patients SET active=0 WHERE id=?', [req.params.id]);
    await run(
      `INSERT INTO audit_logs (user_id,action,module,record_id,details) VALUES (?,?,?,?,?)`,
...
router.post('/:id/documents', async (req, res) => {
  try {
    ...
    const patient = await queryOne('SELECT id, mrn FROM patients WHERE id=? AND active=1', [req.params.id]);
    if (!patient) return res.status(404).json({ error: 'Patient not found' });
    ...
    res.status(201).json(await queryOne('SELECT * FROM patient_documents WHERE id=?', [r.lastId]));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
