  ...
  return `<tr>
    ...
    <td>
      <div class="table-actions">
        ...
        ${App.user?.role === 'admin' ? `
          <button class="btn btn-danger btn-sm" onclick="deleteEmployee(${e.id}, '${e.name}')" title="Delete employee record">Delete</button>
        ` : ''}
      </div>
    </td>
  </tr>`;
...
async function deleteEmployee(id, name) {
  confirmAction('Delete Employee', `Are you sure you want to permanently delete employee "${name}"?`, async () => {
    try {
      await api.delete(`/staff/employees/${id}?permanent=true`);
