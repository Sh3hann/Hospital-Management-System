  ...
  <tr>
    ...
    <td>
      <div class="table-actions">
        ...
        ${!isSelf ? `
          <button class="btn btn-ghost btn-sm ${u.is_active ? 'text-warning' : 'text-success'}" onclick="toggleUserActive(${u.id}, ${u.is_active})">${u.is_active ? 'Deactivate' : 'Activate'}</button>
          <button class="btn btn-danger btn-sm" onclick="deleteUserPermanently(${u.id}, '${u.username}')" title="Delete employee account">Delete</button>
        ` : ''}
      </div>
    </td>
  </tr>`;
...
async function deleteUserPermanently(id, username) {
  confirmAction('Delete Account', `Are you sure you want to permanently delete account "${username}"? This will remove the employee login and all associated staff access. This cannot be undone!`, async () => {
    try {
      await api.delete(`/users/${id}?permanent=true`);
      toast('success', `User account "${username}" deleted permanently`);
      Pages.users();
    } catch(e) {
