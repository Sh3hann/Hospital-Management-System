...
router.get('/:id', (req, res) => {
  ...
  try { doctor.schedule = JSON.parse(doctor.schedule || '{}'); } catch(e) {}
  const today = new Date().toISOString().split('T')[0];
  doctor.todays_appointments = query(
...
// POST /api/doctors
router.post('/', (req, res) => {
  const { name, specialization, department_id, phone, email, qualification, experience_years, consultation_fee, schedule } = req.body;
  ...
  const r = run(
    `INSERT INTO doctors (name,specialization,department_id,phone,email,qualification,experience_years,consultation_fee,schedule)
     VALUES (?,?,?,?,?,?,?,?,?)`,
    [name, specialization, department_id, phone, email, qualification, experience_years || 0,
     consultation_fee || 0, typeof schedule === 'string' ? schedule : JSON.stringify(schedule || {})]
  );
  ...
  res.status(201).json(queryOne('SELECT * FROM doctors WHERE id=?', [r.lastId]));
});

// PUT /api/doctors/:id
router.put('/:id', (req, res) => {
  const doc = queryOne('SELECT id FROM doctors WHERE id=? AND active=1', [req.params.id]);
  if (!doc) return res.status(404).json({ error: 'Doctor not found' });
  const { name, specialization, department_id, phone, email, qualification, experience_years, consultation_fee, schedule } = req.body;
  run(`UPDATE doctors SET name=?,specialization=?,department_id=?,phone=?,email=?,qualification=?,
    experience_years=?,consultation_fee=?,schedule=? WHERE id=?`,
    [name, specialization, department_id, phone, email, qualification, experience_years,
     consultation_fee, typeof schedule === 'string' ? schedule : JSON.stringify(schedule || {}), req.params.id]
  );
  run(`INSERT INTO audit_logs (user_id,action,module,record_id,details) VALUES (?,?,?,?,?)`,
    [req.user.id, 'UPDATE', 'Doctors', req.params.id, `Updated doctor #${req.params.id}`]);
  res.json(queryOne('SELECT * FROM doctors WHERE id=?', [req.params.id]));
});

// DELETE /api/doctors/:id
router.delete('/:id', (req, res) => {
  run('UPDATE doctors SET active=0 WHERE id=?', [req.params.id]);
  run(`INSERT INTO audit_logs (user_id,action,module,record_id,details) VALUES (?,?,?,?,?)`,
...
router.get('/:id/availability', (req, res) => {
  ...
  const doctor = queryOne('SELECT schedule FROM doctors WHERE id=?', [req.params.id]);
  if (!doctor) return res.status(404).json({ error: 'Doctor not found' });

  const days = ['sunday','monday','tuesday','wednesday','thursday','friday','saturday'];
  const dayName = days[new Date(date).getDay()];
  let schedule = {};
  try { schedule = typeof doctor.schedule === 'string' ? JSON.parse(doctor.schedule || '{}') : (doctor.schedule || {}); } catch(e) {}
  const dayHours = schedule[dayName] || '';

  const bookedSlots = query(
  ...
  if (dayHours && dayHours.includes('-')) {
    ...
    if (start && end) {
      ...
      if (!isNaN(sh) && !isNaN(sm) && !isNaN(eh) && !isNaN(eh)) {
